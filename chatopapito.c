...
void boastar(int start, int goal) {
	FILE* f;
...
/* ------------------------------------------------------------------------------*/
void callboastar(int start, int goal) {
	// Realiza configuraciones adicionales si es necesario
    printf("Llamando a boastar con start: %d y goal: %d\n", start, goal);
    float runtime;
...
int main() {
    ...
    int start, goal;
    int intermediates[5]; // Máximo de 5 destinos intermedios
    int num_intermediates;

    while (fscanf(file, "%d", &start) != EOF) {
        // Leer los destinos intermedios y el destino final
        num_intermediates = 0;
        for (int i = 0; i < 5; i++) {
            if (fscanf(file, "%d", &intermediates[i]) == 1) {
                num_intermediates++;
            } else {
                break;
            }
        ...
        // Ejecutar BOA* en la secuencia correcta
        int current_start = start;
        for (int i = 0; i < num_intermediates; i++) {
            callboastar(current_start, intermediates[i]);
            current_start = intermediates[i];
        }
        callboastar(current_start, goal);
    }
