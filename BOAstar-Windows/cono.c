...
// Waypoints
unsigned waypoints[MAXNODES];
unsigned num_waypoints = 0;

//********************************************** Binary Heap Data Structures ******************************************************
// ... (Sección de estructuras y funciones de Binary Heap sin cambios) ...
...
//********************************************** Reading the file ******************************************************
// ... (Funciones de lectura y creación de grafo sin cambios) ...
...
int boastar_with_waypoints() {
    ...
    for (i = 0; i < num_waypoints; ++i) {
        ...
        backward_dijkstra(2);
        boastar();
        start = goal; // Actualizamos el inicio al waypoint actual
    }
    goal = waypoints[num_waypoints]; // Último destino es el verdadero objetivo
    initialize_parameters();
    backward_dijkstra(1);
    backward_dijkstra(2);
    return boastar();
...
/* ------------------------------------------------------------------------------*/
void call_boastar_with_waypoints() {
    float runtime;
    struct timeval tstart, tend;

    gettimeofday(&tstart, NULL);
	
    boastar_with_waypoints();
...
/*----------------------------------------------------------------------------------*/
int main() {
    start = 180833;  // Nodo inicial
    goal = 83149;    // Nodo final

    // Waypoints definidos
    waypoints[0] = 50000;  // Nodo intermedio 1
    waypoints[1] = 120000; // Nodo intermedio 2
    num_waypoints = 2;     // Número de waypoints

    read_adjacent_table("NY-road-d.txt");
    new_graph();

    call_boastar_with_waypoints();
}
