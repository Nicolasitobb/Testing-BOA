...
void leer_instancias(const char *filename, int *start, int *goal, int **intermedios, int *num_intermedios) {
    FILE *f;
    char linea[100];
    f = fopen(filename, "r");
    if (f == NULL) {
...
void call_boastar_with_intermedios(int start, int goal, int *intermedios, int num_intermedios) {
    float runtime;
    struct timeval tstart, tend;
    unsigned long long min_cost;
    unsigned long long min_time;
    ...
    for (int i = 0; i < num_intermedios; i++) {
        int current_goal = intermedios[i];
    ...
    // Final segment from last intermediate to final goal
    start_state = &graph_node[current_start];
    goal_state = &graph_node[goal];
...
int main() {
    int start, goal, *intermedios, num_intermedios;
    leer_instancias("NY-queries-1p.txt", &start, &goal, &intermedios, &num_intermedios);

    read_adjacent_table("NY-queries-1p.txt");
    new_graph();

    call_boastar_with_intermedios(start, goal, intermedios, num_intermedios);
