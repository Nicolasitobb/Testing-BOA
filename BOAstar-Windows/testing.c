...
void initialize_parameters(int start_in, int goal_in) {
    start = start_in;
    goal = goal_in;
...
int boastar(int *intermedios, int num_intermedios) {
  FILE* f;
  f = fopen("salida344.csv", "w");
    ...
    while (topheap() != NULL) {
      ...
      // Verificar si el nodo actual es un intermedio
      int es_intermedio = 0;
      for (int i = 0; i < num_intermedios; i++) {
        if (n->state == intermedios[i]) {
          es_intermedio = 1;
          break;
        }
      }

      if (n->state == goal || es_intermedio) {
...
/* ------------------------------------------------------------------------------*/
void call_boastar(int start_in, int goal_in, int *intermedios, int num_intermedios) {
    float runtime;
    struct timeval tstart, tend;
    unsigned long long min_cost;
    unsigned long long min_time;

    initialize_parameters(start_in, goal_in);
  ...
  //BOA*
    boastar(intermedios, num_intermedios);
...
void leer_instancias(const char *filename) {
    ...
    while (fgets(linea, 100, f) != NULL) {
        ...
        intermedios = (int *)malloc((num_intermedios - 1) * sizeof(int));
        if (intermedios == NULL) {
            printf("Error al asignar memoria.\n");
            exit(1);
        }

        // Rebobinar strtok y leer los valores
        rewind(f); 
        fgets(linea, 100, f); // Leer la línea de nuevo
        strtok(linea, " "); // Saltar el primer valor (start)
        for (int i = 0; i < num_intermedios - 1; i++) {
        ...
        // Llamar a call_boastar con los parámetros de la instancia
        call_boastar(start, goal, intermedios, num_intermedios - 1); 
...
int main() {
  ...
  new_graph();  
  leer_instancias("instancias.txt"); 
  return 0;
}
